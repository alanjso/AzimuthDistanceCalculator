...
class AzimuthsAndDistancesDialog(QDialog, FORM_CLASS):
    ...
    def fillTable(self):
        ...
        for i in range(0,len(distancesAndAzimuths)):
            ...
            itemVertex = QTableWidgetItem(prefix+str(i+1))
            self.tableWidget.setItem(i, 0, itemVertex)
            n = Decimal(self.points[i].x()).quantize(q)
            itemN = QTableWidgetItem(str(n))
            self.tableWidget.setItem(i, 1, itemN)
            e = Decimal(self.points[i].y()).quantize(q)
            itemE = QTableWidgetItem(str(e))
            self.tableWidget.setItem(i, 2, itemE)

            if (i == len(distancesAndAzimuths) - 1) and isClosed:
                itemSide = QTableWidgetItem(prefix+str(i+1)+"-{}1".format(prefix))
                self.tableWidget.setItem(i, 3, itemSide)
            else:
                itemSide = QTableWidgetItem(prefix+str(i+1)+"-{}".format(prefix)+str(i+2))
                self.tableWidget.setItem(i, 3, itemSide)
            ...
            self.tableWidget.setItem(i, 5, itemDistance)
            itemConfronting = QTableWidgetItem("")
            self.tableWidget.setItem(i, 6, itemConfronting)
